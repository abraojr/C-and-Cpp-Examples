...
Stack<T, max_size>::Stack() : current_size(0), elements(new T[max_size]) {}

template <typename T, int max_size>
Stack<T, max_size>::Stack(initializer_list<T> list) : current_size(list.size()), elements(new T[list.size()])
{
    if (max_size < current_size)
    {
        throw invalid_argument("Max size should be at list equal to the current size");
    }
...
template <typename T, int max_size>
Stack<T, max_size>::Stack(const Stack<T, max_size> &other) : current_size(other.current_size)
{
